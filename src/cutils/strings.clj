...
(defn pstr
  "Cleans up string given as first argument. Returns the string or nil."
  [s]
  (some-> s str (s/replace #"(^\s+)|(\s+$)" "") not-empty))

(def remove-white-chars
  [s]
  (s/replace s #"\s" ""))

(defn str->int
  "Converts string to integer. Returns nil if something went wrong."
  [s]
  (if (number? s)
    s
    (some->> s pstr
